...
Torch-only: this module assumes GPU-only operation, so `D`/`x` are
always torch.Tensor (CPU or CUDA), not numpy arrays.
"""
...
def lipschitz_constant_lsq(D: torch.Tensor) -> float:
    """
    ...
    This is a one-time precomputation, so the SVD cost is paid
    once regardless of how many FISTA iterations follow.
    """
    sigma_max = torch.linalg.svdvals(D)[0]
